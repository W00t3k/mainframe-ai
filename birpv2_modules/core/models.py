...
class Screen:
	...
	def stringbuffer(self):
		...
		for line in self.plainbuffer:
			hexstr = ''.join(line)
			newstr = bytes.fromhex(hexstr).decode('latin-1')
			...
			for i in line.split(' '):
				...
				if len(i) > 3 and i.find('SF(') >= 0:
					...
					if i == '00':
						...
						newline.append(bytes.fromhex(i).decode('latin-1'))
		...
		for line in self.rawbuffer:
			...
			for i in line.split(' '):
				...
				# Add the character to the last field entity added
					...
					contents = bytes.fromhex(i).decode('latin-1')
					if len(field_list) > 0:
	...
	def protected_fields(self):
		try:
			res = list(filter(lambda x: x.protected == 1, self.fields))
		except IndexError:
			res = []
		return res
	...
	def input_fields(self):
		try:
			res = list(filter(lambda x: x.protected == 0, self.fields))
		except IndexError:
			res = []
		return res
	...
	def hidden_fields(self):
		try:
			res = list(filter(lambda x: x.hidden == 1, self.fields))
		except IndexError:
			res = []
		return res
	...
	def modified_fields(self):
		try:
			res = list(filter(lambda x: x.modify == 1, self.fields))
		except IndexError:
			res = []
		return res

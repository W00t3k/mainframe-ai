...
check_tk5_http() {
  (echo > /dev/tcp/127.0.0.1/8038) 2>/dev/null
}

tk5_ready() {
  local ready_re='TCAS ACCEPTING LOGONS|LOGON APPLID|IKT005I|READY'
  grep -Eq "$ready_re" "$TK5/log/hardcopy.log" "$LOGDIR/hercules.log" 2>/dev/null
}

tk5_fatal_error() {
  grep -Eq \
    'HHCCP011I CPU[0-9]+: Disabled wait state|HHCPN995E Script file .+ not found|IEA305A INVALID REPLY FORMAT|HHCHT002E socket: Operation not permitted|HHCDA005E .+ open error: Permission denied|HHCSD013E Error creating socket|HHCCT007E .+ Error creating socket|HHCCA003E .+ Cannot obtain socket|Syntax error|cannot open shared object|HHCDA020E|HHCDA001E|open error' \
    "$LOGDIR/hercules.log" 2>/dev/null
}
...
wait_for_tk5_ready() {
  ...
  while [ $elapsed -lt $max_wait ]; do
    tk5_fatal_error && return 2
    if check_tk5 && tk5_ready; then
      return 0
    fi
    sleep 5
    elapsed=$((elapsed + 5))
...
tk5_has_extra_terminal_devices() {
  grep -Eq '^(0400[[:space:]]+3270|0400-041F[[:space:]]+3270|0400\.32[[:space:]]+3270)' \
    "$TK5/conf/tk5-linux.cnf" "$TK5/conf/tk5.cnf" 2>/dev/null
}

prepare_linux_ipl_rc() {
  RUNTIME_IPL_RC="$LOGDIR/ipl-linux-compat.rc"
  cat > "$RUNTIME_IPL_RC" <<'EOF'
# Linux Hercules 3.13 does not expand ${...:=...} inside HAO commands.
# Use concrete script paths so the unattended IPL replies actually fire.
hao tgt MVS038J
hao cmd script scripts/tk5.rc
hao tgt IEA101A
hao cmd script scripts/SCR101A_default
hao tgt IEA305A
hao cmd script scripts/SCR101A_default
pause 4
ipl 390
...
req = urllib.request.Request(
    'http://127.0.0.1:8038/cgi-bin/tasks/syslog',
    data=data,
    method='POST',
)
...
bootstrap_linux_tk5() {
  local cmd
  wait_for "Hercules HTTP" check_tk5_http 60 || return 1

  for cmd in \
    "hao tgt ^MVS038J" \
    "hao cmd script scripts/tk5.rc" \
    "hao tgt ^IEA101A " \
    "hao cmd script scripts/SCR101A_specific02" \
    "hao tgt ^IEA305A " \
    "hao cmd script scripts/SCR101A_default" \
    "hao tgt ^HHC00010A" \
    "hao cmd script scripts/SCR00010A" \
    "ipl 390"
  do
    hercules_http_cmd "$cmd" || return 1
    sleep 1
  done
...
start_tk5_svc() {
  ...
  if check_tk5; then
    if wait_for_tk5_ready 10; then
      ok "TK5 already running and ready on port 3270"
      TK5_OK=1
      return 0
    fi
    fail "Port 3270 is open but MVS is not in a usable logon/READY state"
    info "Run ./start.sh --kill before restarting TK5"
    TK5_OK=0
    return 1
  fi
  ...
  mkdir -p "$TK5/log" "$TK5/prt" "$TK5/pch" 2>/dev/null
  ...
  if [ "$(uname -s)" = "Linux" ]; then
    ...
    nohup bash -c "
      ...
      exec \"$HERC_BIN/hercules\" -f conf/tk5-linux.cnf -d
    ...
    detect_python
    if bootstrap_linux_tk5; then
      info "Linux IPL sequence submitted via Hercules HTTP console"
    else
      fail "Failed to submit Linux IPL sequence via Hercules HTTP console"
  ...
  if [ "$(uname -s)" = "Linux" ]; then
    # Linux: wait for real guest readiness, not just an open TN3270 socket
    TK5_OK=0
    for _i in $(seq 5 5 300); do
      if wait_for_tk5_ready 5; then
        ok "TK5 started — TN3270 on port 3270"
        ok "MVS reached a usable logon/READY state"
      ...
      rc=$?
      if [ "$rc" = "2" ]; then
        fail "Hercules/MVS fatal startup error detected"
        cat "$LOGDIR/hercules.log" 2>/dev/null | sed 's/^/    /'
        TK5_OK=0
        return 1
      fi
      echo "  … TN3270 is up but MVS is not ready yet (${_i}s) — last log line:"
      tail -1 "$LOGDIR/hercules.log" 2>/dev/null | sed 's/^/    /'
    done
    if [ "$TK5_OK" = "0" ]; then
      fail "TK5 failed to reach a usable logon/READY state after 300s"
...
start_webapp_svc() {
  ...
  _auto_connect_tk5() {
    ...
    # Phase 1: Wait for real guest readiness, not just a listening socket
    wait_for_tk5_ready 600 || return 1
    sleep 1
